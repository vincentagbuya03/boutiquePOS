...
<div class="overview-header">
    <h1 class="overview-title">Business Overview</h1>
...
<div class="stats-main-grid">
    ...
    <div class="stat-arch-card">
        ...
        <div class="stat-arch-meta stat-meta-neutral" style="color: #802030;">
    ...
    <div class="stat-arch-card">
        <div class="stat-arch-label">Total Products</div>
        <div class="stat-arch-value">{{ number_format($totalProducts) }}</div>
        <div class="stat-arch-meta stat-meta-positive">
            <i class="fas fa-archive"></i> San Carlos Branch
        </div>
    ...
    <div class="stat-arch-card">
        ...
        <i class="fas fa-hand-holding-usd stat-ghost-icon"></i>
    </div>

    <div class="stat-arch-card">
        <div class="stat-arch-label">Total Staff</div>
        <div class="stat-arch-value">{{ $totalUsers }}</div>
        <div class="stat-arch-meta stat-meta-neutral">
            <i class="fas fa-users"></i> Curated workforce
        </div>
        <i class="fas fa-user-tie stat-ghost-icon"></i>
    </div>
</div>
...
<div class="performance-grid">
    ...
    <div class="seasonal-archive-card" style="background: linear-gradient(rgba(26, 26, 26, 0.85), rgba(26, 26, 26, 0.85)), url('https://images.unsplash.com/photo-1549298916-b41d501d3772?auto=format&fit=crop&q=80&w=800');">
        <div>
            @php 
                $outOfStockCount = \App\Models\Inventory::where('quantity', 0)->count();
                $lowStockCount = \App\Models\Inventory::where('quantity', '>', 0)->where('quantity', '<=', \DB::raw('reorder_level'))->count();
            @endphp
            <h3 class="seasonal-title">Inventory Health:<br>Real-time Pulse</h3>
            <p class="seasonal-desc">
                Currently, <strong>{{ $outOfStockCount }}</strong> pieces are out of stock and <strong>{{ $lowStockCount }}</strong> require immediate curation in San Carlos.
            </p>
        </div>
        <a href="{{ route('inventory.index') }}" class="seasonal-action-btn">Manage Restock</a>
    </div>
</div>
...
<div class="orders-section-card">
    <div class="section-header-flex">
        ...
        <span style="font-size: 0.75rem; font-weight: 800; color: #adb5bd; text-transform: uppercase;">Real-time sync</span>
    </div>

    <table class="arch-table">
        <thead>
            <tr>
                <th>Customer / Curator</th>
                <th>Total Revenue</th>
                <th>Status</th>
                <th>Action</th>
            </tr>
        ...
        <tbody>
            @php $recentSales = \App\Models\Sale::with('user')->latest('date_sold')->limit(5)->get(); @endphp
            @foreach($recentSales as $sale)
            <tr>
                <td>
                    <div class="customer-profile">
                        <div class="avatar-mini" style="background: #fdf2f4; color: #802030;">{{ substr($sale->user->name ?? 'C', 0, 1) }}</div>
                        <span>{{ $sale->user->name ?? 'Guest Curator' }}</span>
                    </div>
                </td>
                <td style="font-weight: 800; color: #1a1a1a;">₱{{ number_format($sale->total_amount, 2) }}</td>
                <td><span class="status-arch-badge status-ship">Completed</span></td>
                <td><a href="{{ route('sales.show', $sale) }}"><i class="fas fa-eye action-ellipsis"></i></a></td>
            </tr>
            @endforeach
        </tbody>
    </table>
...
<div class="orders-section-card" style="margin-top: 2.5rem; border-color: #fdf2f2;">
    <div class="section-header-flex">
        <h2 class="section-title" style="color: #802030;"><i class="fas fa-exclamation-circle"></i> Inventory Alerts</h2>
    </div>
    <table class="arch-table">
        <thead>
            <tr>
        ...
        <tbody>
            ...
            <tr>
                ...
                <td style="color: #802030; font-weight: 800;">{{ $item->quantity }}</td>
                <td>{{ $item->reorder_level }} units</td>
                <td><span class="status-arch-badge status-proc">Urgent Restock</span></td>
            </tr>
            @endforeach
        </tbody>
    </table>
